...
Trades ProRataOrderBook::AddOrder(const Order& order) {
    if (order.q_ <= 0 || index_.contains(order.id_))
        return {};
    return order.isBuy_ ? match_(order, asks_, bids_)
                        : match_(order, bids_, asks_);
}
...
void ProRataOrderBook::CancelOrder(const Id orderId) {
    ...
    index_.erase(it);
    loc.isBuy_ ? remove_(bids_, loc.p_, orderId)
               : remove_(asks_, loc.p_, orderId);
}
...
void ProRataOrderBook::fillLevel_(Order& order, Price px,
                                  Level& level, Trades& trades) {
    Quantity& rem = order.q_;
    auto& queue = level.queue_;

    // Aggressor swallows the whole level
    if (rem >= level.total_) {
        for (auto& [id, resting] : queue) {
            trades.push_back(Trade{id, order.id_, order.id_,
                                   order.isBuy_, px, resting.q_});
            index_.erase(id);
        }
        rem -= level.total_;
        level.total_ = 0;
    ...
    // floor allocation then distribute the rounding leftover.
    std::vector<Alloc> allocs;
    allocs.reserve(queue.size());

    Quantity allocated = 0;
    for (const auto& [id, resting] : queue) {
        const Quantity share = static_cast<Quantity>(
            static_cast<i64>(rem) * resting.q_ / level.total_);
    ...
    if (leftover > 0) {
        // Largest resting size first, ties by time priority (lower
        // Id).
        ...
        std::stable_sort(by_size.begin(), by_size.end(),
                         [&](std::size_t a, std::size_t b) {
                             return allocs[a].resting >
                                    allocs[b].resting;
                         });
        for (std::size_t k = 0; k < by_size.size() && leftover > 0;
             ++k) {
    ...
    for (const Alloc& a : allocs) {
        if (a.fill == 0)
            continue;
        trades.push_back(Trade{a.id, order.id_, order.id_,
                               order.isBuy_, px, a.fill});
