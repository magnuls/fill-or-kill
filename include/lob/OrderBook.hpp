...
// Interface every book implementation exposes. Strategies program against this.
class OrderBook {
  public:
    ...
    // Resting quantity at a price on the given side; 0 if the level is empty.
    virtual Quantity LevelTotal(Price p, bool bidSide) const = 0;
    virtual bool Contains(Id id) const = 0;
};

#endif // LOB_ORDERBOOK_HPP
